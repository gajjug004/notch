...
- **Single window, list/detail SPA**: one frameless/transparent/always-on-top window labeled `"main"` (`window.rs:MAIN_LABEL`, `open_main_window`). The frontend (`main.ts`) swaps between a **list view** (one row per task, live timer + schedule badge) and a **detail view** (the sticky-note editor) in the same window — no per-task OS windows.
...
- **Settings window**: separate page (`settings.html` / `settings.ts`), label `"settings"` — default countdown, sound, autostart, global pause. Global pause freezes all timers, persisted.

Frontend↔backend contract: TS calls `invoke(...)` for commands; Rust pushes `listen`-able events (above + `global-pause`). Keep `src/types.ts` in sync with `task.rs` / `timer.rs` / `schedule.rs` serde shapes — they are mirrored by hand.
